...
Array.min=function(array,callback){
 ...
 var x=+Infinity;for(var i=0;i < array.length;i++){
...
Array.sorted=function(array,reverse,key){
 ...
 array=array.slice();array=array.sort(cmp);if(reverse)array=array.reverse();return array;};
...
Array.len=function(array){
 return array.length;};
var sum=Array.sum;var map=Array.map;var filter=Array.filter;var enumerate=Array.enumerate;var sorted=Array.sorted;var reversed=Array.reversed;var unique=Array.unique;var shuffled=Array.shuffled;var choice=Array.choice;var range=Array.range;var len=Array.len;
...
Math.sum=function(a){
 var n=0;for(var i=0;i < a.length;i++)n+=a[i];return n;};
...
Math.smoothrange=function(a,b,n){
 ...
 var f=v/m;
 if(round){
 if(f < 1.5)return m*1;if(f < 3.0)return m*2;if(f < 7.0)return m*5;}else{
 if(f <=1.0)return m*1;if(f <=2.0)return m*2;if(f <=5.0)return m*5;
 }
 return m*10;}
 ...
 if(a===b){
 return[a];}
 var r=_multiple(b-a);var t=_multiple(r/(n-1),true);a=Math.floor(a/t)*t;b=Math.ceil( b/t)*t;var rng=[];for(var i=0;i <(b-a)/t+1;i++){
 rng.push(a+i*t);}
 return rng;};
...
var geometry=new Geometry();var geo=geometry;
Math.angle=geometry.angle;Math.distance=geometry.distance;Math.coordinates=geometry.coordinates;
var RGB="RGB";var HSB="HSB";var HCL="HCL";var HEX="HEX";
var Color=Class.extend({
 init: function(r,g,b,a,options){
 var base=options && options.base || 1.0;var colorspace=options && options.colorspace || RGB;
 ...
 var base=options && options.base || 1.0;var colorspace=options && options.colorspace || RGB;var r=this.r;var g=this.g;var b=this.b;var a=this.a;if(colorspace==HEX){
 ...
 var hsb=_rgb2hsb(this.r,this.g,this.b);hsb[0]+=options.hue || 0;hsb[1]*=options.saturation || 1;hsb[2]*=options.brightness || 1;return new Color(Math.mod(hsb[0],1),hsb[1],hsb[2],this.a,{"colorspace": HSB});}
});
function rgb(r,g,b){
...
function _rgb2hsb(r,g,b){
 ...
 h=Math.mod(h/6.0,1);return[h,s,v];}
function _hsb2rgb(h,s,v){
 if(s==0){
 return[v,v,v];}
 h=Math.mod(h,1)*6.0;var i=Math.floor(h);var f=h-i;var x=v*(1-s);var y=v*(1-s*f);var z=v*(1-s*(1-f));if(i > 4){
 return[v,x,y];}
 return[[v,z,x],[y,v,x],[x,v,z],[x,y,v],[z,x,v]][parseInt(i)];}
function _rgb2xyz(r,g,b){
 r=((r > 0.04045)? Math.pow((r+0.055)/1.055,2.4): r/12.92)*100;g=((g > 0.04045)? Math.pow((g+0.055)/1.055,2.4): g/12.92)*100;b=((b > 0.04045)? Math.pow((b+0.055)/1.055,2.4): b/12.92)*100;
 var x=r*0.4124+g*0.3576+b*0.1805;var y=r*0.2126+g*0.7152+b*0.0722;var z=r*0.0193+g*0.1192+b*0.9505;return[x,y,z];}
function _xyz2rgb(x,y,z){
 x/=100;y/=100;z/=100;var r=x*3.2406+y*-1.5372+z*-0.4986;var g=x*-0.9689+y*1.8758+z*0.0415;var b=x*0.0557+y*-0.2040+z*1.0570;r=(r > 0.0031308)? 1.055*Math.pow(r,1/2.4)-0.055 : 12.92*r;g=(g > 0.0031308)? 1.055*Math.pow(g,1/2.4)-0.055 : 12.92*g;b=(b > 0.0031308)? 1.055*Math.pow(b,1/2.4)-0.055 : 12.92*b;return[r,g,b];}
function _xyz2lab(x,y,z){
 x/=95.047;y/=100.000;z/=108.883;x=(x > 0.008856)? Math.pow(x,1/3): 7.787*x+(16/116);y=(y > 0.008856)? Math.pow(y,1/3): 7.787*y+(16/116);z=(z > 0.008856)? Math.pow(z,1/3): 7.787*z+(16/116);var l=116*y-16;var a=500*(x-y);var b=200*(y-z);return[l,a,b];}
function _lab2xyz(l,a,b){
 var y=(l+16 )/116;var x=a/500+y;var z=y-b/200;x=(Math.pow(x,3)> 0.008856)? Math.pow(x,3):(x-16/116)/7.787;y=(Math.pow(y,3)> 0.008856)? Math.pow(y,3):(y-16/116)/7.787;z=(Math.pow(z,3)> 0.008856)? Math.pow(z,3):(z-16/116)/7.787;
 x*=95.047;y*=100.000;z*=108.883;return[x,y,z];}
function _lab2lch(l,a,b){
 var h=Math.atan2(b,a);h=(h > 0)? h/Math.PI*180 : 360-Math.abs(h)/Math.PI*180;c=Math.sqrt(a*a+b*b);return[l,c,h];}
function _lch2lab(l,c,h){
 h=h/180*Math.PI;var a=Math.cos(h)*c;var b=Math.sin(h)*c;return[l,a,b];}
function _rgb2hcl(r,g,b){
 var xyz=_rgb2xyz(r,g,b);var lab=_xyz2lab(xyz[0],xyz[1],xyz[2]);var lch=_lab2lch(lab[0],lab[1],lab[2]);lch[0]/=100;lch[1]/=100;lch[2]/=360;return[lch[2],lch[1],lch[0]];}
function _hcl2rgb(h,c,l){
 h*=360;c*=100;l*=100;var lab=_lch2lab(l,c,h);var xyz=_lab2xyz(lab[0],lab[1],lab[2]);return _xyz2rgb(xyz[0],xyz[1],xyz[2]);}
function _rgba2rgb(r,g,b,a,blend){
 blend=blend ||[1,1,1];r=r*a+(1-a)*blend[0];g=g*a+(1-a)*blend[1];b=b*a+(1-a)*blend[2];a=1;return[r,g,b,a];}
...
function colors(colors,k,colorspace){
 if(k===undefined)k=100;if(colorspace===undefined)colorspace=HCL;var a=Array.map(colors,function(clr){ return clr.map({colorspace: colorspace});});var b=[];var x,y,t,n=a.length-1;for(var i=0;i < k-1;i++){
 x=Math.min(Math.floor(i/k*n),n);y=Math.min(x+1,n);t=(i-x*k/n)/(k/n);b.push(new Color(
...
function _rotateRYB(h,s,b,angle){
 if(angle===undefined)angle=180;h=Math.mod(h*360,360);
 ...
 a=Math.lerp(x0,x1,(h-y0)/(y1-y0));break;}
 }
 a=Math.mod(a+angle,360);for(var i=0;i < wheel.length-1;i++){
 x0=wheel[i][0];x1=wheel[i+1][0];y0=wheel[i][1];y1=wheel[i+1][1];if(x0 <=a && a <=x1){
 h=Math.lerp(y0,y1,(a-x0)/(x1-x0));break;}
 }
 return[h/360.0,s,b];}
...
function _colorMixin(options){
 var s=_ctx.state;var o=options;if(o===undefined){
...
var Gradient=Class.extend({
 ...
 this.spread=Math.max((o.spread !==undefined)? o.spread : 100,0);this.angle=o.angle || 0;},
 _get: function(dx,dy){
...
var Path=BezierPath=Class.extend({
 ...
 var a1=angle1 % 360;var a2=angle2;if(a2 < a1){
 a2=a2+360;}
 if(options && options.clockwise===false){
 a2=a2 % 360-360;}
 var points=_bezier.arc(x-radius,y-radius,x+radius,y+radius,a1,a2-a1);for(var i=0;i < points.length;i++){
 var pt=points[i];if(i==0){
 ...
 if(precision===undefined)precision=10;return _bezier.length(this,false,precision);},
...
function arc(x,y,radius,angle1,angle2,options){
 ...
 var a1=Math.radians(angle1);var a2=Math.radians(angle2);_ctx.beginPath();_ctx.arc(x,y,radius,a1,a2,(options && options.clockwise===false));_ctx_stroke(a[1],a[2],a[3],a[4]);}
}
...
function star(x,y,points,outer,inner,options){
 ...
 var r=(i % 2==0)? outer : inner;var a=Math.PI*i/points;p.lineto(
 x+r*Math.sin(a),
 y+r*Math.cos(a)
 );};p.closepath();p.draw(options);}
...
var Image=Class.extend({
 ...
 var o=options ||{};this._url=url;this._img=_cache.image(url);this.x=o.x || 0;this.y=o.y || 0;this.width=(o.width !==undefined)? o.width : null;this.height=(o.height !==undefined)? o.height : null;this.alpha=(o.alpha !==undefined)? o.alpha : 1.0;
...
function adjust(img,options){
 ...
 var hsb=_rgb2hsb(p[0]/255,p[1]/255,p[2]/255);var rgb=_hsb2rgb(Math.clamp(Math.mod(hsb[0]+m,1),0,1),hsb[1],hsb[2]);return[rgb[0]*255,rgb[1]*255,rgb[2]*255,p[3]];
 });}
...
(luminance > 0.55)? b :
 Math.lerp(a,b,(luminance-0.45)*255);};break;default:
 op=function(x,y){ return 0;};}
 function mix(p1,p2,op,luminance){
 var p=[0,0,0,0];var a=p2[3]/255*alpha;p[0]=Math.lerp(p1[0],op(p1[0],p2[0],luminance),a);p[1]=Math.lerp(p1[1],op(p1[1],p2[1],luminance),a);p[2]=Math.lerp(p1[2],op(p1[2],p2[2],luminance),a);p[3]=Math.lerp(p1[3],255,a);return p;}
...
attachEvent(window,"load",function(){
 ...
 var i=this.i;
 var e=this.e;if(e[i].className=="canvas" || e[i].type=="text/canvas"){
 var canvas=e[i].getAttribute("target")||
 e[i].getAttribute("canvas");if(canvas){
 canvas=document.getElementById(canvas);}else{
 canvas=document.createElement("canvas");canvas.className="canvas";canvas.width=parseInt(e[i].getAttribute("width")|| 500);canvas.height=parseInt(e[i].getAttribute("height")|| 500);
